...
def sentiment_duplicates():
    dfs = [pd.read_csv(f, dtype={'id': str}) for f in glob.glob("sentiment_results/*.csv")]
    print("ALL RES READ")
...
def plot_all_sentiment():
    ...
    sentiment_counts = sentiment_counts[sentiment_counts['total'] > 10]
    ...
    events = {
        ...
        "Assassination Attempt": pd.Timestamp('2024-07-13')
    }

    plt.figure()
    for event, dt in events.items():
        plt.axvline(x=dt, linestyle='--', linewidth=2, label=event)
    ...
    plt.title('Total Tweet Sentiment')
    plt.legend()
...
def plot_topics():
    df = pd.read_csv("new_merged_sentiment_topic.csv")
    ...
    sentiment_counts = sentiment_counts[sentiment_counts['total'] > 6]
    ...
    plt.title(f"{topic[1]} : {sub_df.shape[0]} Tweets as a Proportion of Daily Total")
    plt.legend()
